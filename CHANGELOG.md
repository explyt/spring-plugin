...
### Spring Core
- feat: Report `System.getenv(...)` and `System.getProperty(...)` inside a Spring bean, which read a setting from the JVM instead of the `Environment`: the value cannot be overridden per profile, by an external configuration source or by a test property source, it is invisible to the plugin's property resolution and it breaks as soon as the same setting is also declared under a relaxed name. `System.getenv` is a warning; `System.getProperty` is a weak warning and only when the key is also declared in one of the module's configuration files, so JVM properties such as `java.io.tmpdir` stay unreported. Both are reported only in a bean class and never in `static main` or a static initializer, which run before `SpringApplication.run`. Quick-fixes replace the call with a `@Value("${...}")` dependency (a field in Java, a constructor parameter or a `@field:Value` property in Kotlin) or with `environment.getProperty(...)`, injecting an `Environment` when the bean has none (#371)
