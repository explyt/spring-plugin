...
### Spring Core
- fix: Decide whether `management.endpoint.<id>.access` exists from the actuator version on the module's classpath right now, instead of from the one seen when the endpoint cache was first populated: the Spring Boot 3.4 capability check was evaluated outside the `CachedValueProvider` and captured by it, and the platform keeps the provider of the first call for the lifetime of the module — so a first call made before the actuator jar entered the model (a Gradle import, the first highlighting pass after the project opens) left `.access` reported as an unresolved key with no navigation for the rest of the session, and the IDE logged `Incorrect CachedValue use: same CachedValue with different captured context` (#443)
