...
object SqlPsiImplUtil {
    @JvmStatic
    fun getMapOperationType(expression: SqlMapBasedReferenceExpression): JpqlElementType {
        return expression.node.firstChildNode.elementType as JpqlElementType
    }

    @JvmStatic
    fun getName(element: SqlAliasDeclaration): String {
        return PsiTreeUtil.lastChild(element).text
    }

    @JvmStatic
    fun setName(element: SqlAliasDeclaration, newName: String): PsiElement {
        val oldIdentifier = element.node.findChildByType(JpqlTypes.IDENTIFIER)
            ?: return element

        val jpqlElementFactory = JpqlElementFactory.Companion.getInstance(element.project)
