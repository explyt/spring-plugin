...
class SpringBoot4TestRestTemplatePackageInspectionTest : ExplytInspectionJavaTestCase() {
    ...
    fun testLegacyFieldTypeReported() {
        myFixture.configureByText(
            ...
            public class MyTest {
                <warning descr="TestRestTemplate moved to org.springframework.boot.resttestclient in Spring Boot 4">TestRestTemplate</warning> template;
            }
            """.trimIndent()
        )
    ...
    fun testLegacyParameterTypeReported() {
        myFixture.configureByText(
            "MyTest.java",
            """
            import org.springframework.boot.test.web.client.TestRestTemplate;
            ...
            public class MyTest {
                void run(<warning descr="TestRestTemplate moved to org.springframework.boot.resttestclient in Spring Boot 4">TestRestTemplate</warning> template) { }
            }
            """.trimIndent()
        )
        myFixture.testHighlighting("MyTest.java")
    ...
    fun testQuickFixUpdatesImport() {
        myFixture.configureByText(
            ...
            public class MyTest {
                TestRest<caret>Template template;
            }
            """.trimIndent()
        )
        val intention = myFixture.availableIntentions
            .firstOrNull { it.text.contains("Spring Boot 4") }
