...
- **Spring Debugger**: Full access to Spring Context in
  debug.[Demo video.](https://github.com/explyt/spring-plugin/wiki/Spring-Debugger)
...
- Current tools include:
    ...
    - [`explyt_get_spring_http_endpoints`](https://github.com/explyt/spring-plugin/blob/main/modules/spring-ai/src/main/kotlin/com/explyt/spring/ai/mcp/SpringMcpProvider.kt) — lists all HTTP endpoints in the project with optional filtering by controller class name or endpoint type (SPRING_MVC, SPRING_WEBFLUX, SPRING_JAX_RS, etc.). Returns an object with `endpoints` (a summary of every endpoint: HTTP method, full path, controller, method name, return type, file location, and endpoint type), `totalCount` (how many endpoints matched the filters), and `truncated` (true when matches exceeded the result cap, so `endpoints` is incomplete — narrow the result with the filters).
